...
class Stega:
    def stega_encrypt(self):
        keys = []
        img = Image.open(input("Enter path to image: "))
    ...
    def main(self, argv):
        ...
        parser.add_argument('-d', help='To decrypt your message from picture', metavar='', type=str)
        parser.add_argument('-e', help='To encrypt message from picture', metavar='', type=str)
        args = parser.parse_args()

        if args.d:
            print("The contained message is: \n", self.stega_decrypt())
        elif args.e:
            self.stega_encrypt()
            print('DONE!!! \n Your message has been contained.')
        else:
            print("ERRROR!!!")
            sys.exit()
